        ...
        <el-dialog v-model="dialogVisible" title="知识库权限" align-center width="500px" style="height: 500px;" >
            ...
            <div class="scroll-container">
                <el-table :data="filteredData" @selection-change="handleSelectedRows" ref="tableRef">
                    <el-table-column type="selection" width="60" :selectable="checkSelect" />
                    ...
                    <el-table-column property="role_name" label="权限" width="120">
                        <template #default=scope>
                            <el-select
                                class="select"
                                :disabled="scope.row.account_id == id || scope.row.account_id == createdBy"
                                v-model="scope.row.role_name"
                                @change="(val:string)=>handlePermissionChange(val, scope.row)">
                                <el-option
...
const checkSelect = (row,index) => {
  return !(row.account_id == id.value || row.account_id == createdBy)
}
...
watch(() => props.visible, (val) => {
    console.log('个人信息',userStore?.getUserInfo)
    if(val) {
        loadData();
    }
})
